...
def val(model):
    ...
    with torch.no_grad():
        ...
        print(opt.name)
        result = io.loadmat(
            './model/' + opt.name + '/' + 'pytorch_result.mat')  # result = scipy.io.loadmat('pytorch_result.mat')
        query_feature = torch.FloatTensor(result['query_f'])
...
def train_model(model, model_test, criterion, optimizer, scheduler, epoch, warm_up, warm_iteration, num_epochs=25):
    ...
    for phase in ['train']:
        ...
        if opt.views == 2:
            print(
                'train Loss: {:.4f} Satellite_Acc: {:.4f}  Drone_Acc: {:.4f}'.format(epoch_loss, epoch_acc, epoch_acc2))
            scheduler.step()
